...
class SendEventService:
    ...
    async def send_event(self, event: EventModel, chat_id: int):
        if len(event.media_items) == 0:
            await self._bot.send_message(chat_id=chat_id, text=event.text)
        elif len(event.media_items) == 1:
            if event.media_items[0].media_type == 'photo':
                await self._bot.send_photo(chat_id=chat_id, photo=event.media_items[0].file_id, caption=event.text)
            if event.media_items[0].media_type == 'video':
                await self._bot.send_video(chat_id=chat_id, video=event.media_items[0].file_id, caption=event.text)
        else:
            await self._bot.send_media_group(chat_id=chat_id, media=[
                ...
                for i, m in enumerate(event.media_items)
            ])
